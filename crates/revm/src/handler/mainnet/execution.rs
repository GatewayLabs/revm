...
use interpreter::{
    ...
    EOFCreateInputs, EOFCreateKind, Gas, InterpreterAction, InterpreterResult, NewFrameAction,
...
pub fn execute_frame<EvmWiringT: EvmWiring, SPEC: Spec>(
    ...
    let priv_memory: PrivateMemory = mem::replace(private_memory, EMPTY_PRIVATE_MEMORY);
    let next_action = match instruction_tables {
        InstructionTables::Plain(table) => interpreter.run(memory, priv_memory, table, context),
        InstructionTables::Boxed(table) => interpreter.run(memory, priv_memory, table, context),
    };
    // Take the shared memory back.
    *shared_memory = interpreter.take_memory();
    *private_memory = interpreter.take_private_memory();
